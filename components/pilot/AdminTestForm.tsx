...
const ADMIN_STEPS: { key: string; label: string }[] = [
  { key: "vendor_queue",       label: "Vendor Queue" },
  { key: "approval_process",   label: "Approval Process" },
  { key: "quote_pipeline",     label: "Quote Pipeline" },
...
export function AdminTestForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!adminName || !email) { toast.error("Name and email are required"); return; }
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="pt-24 pb-20 flex flex-col items-center justify-center min-h-[80vh]">
          ...
          <p className="text-gray-500 text-lg text-center max-w-md">
            Your admin test results have been submitted. Your feedback directly informs the launch readiness score.
          </p>
        </div>
        <Footer />
      </div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="pt-16" style={{ background: "#0D1B3E" }}>
        <div className="max-w-3xl mx-auto px-4 py-16 text-center">
          ...
          <h1 className="text-3xl font-light text-white mb-3">Admin Journey Test</h1>
          <p className="text-white/50 text-sm max-w-md mx-auto">
            Test the admin command centre and report what you find. All results feed the launch readiness tracker.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl mx-auto px-4 py-12 space-y-8">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input value={adminName} onChange={(e) => setAdminName(e.target.value)} placeholder="Your name" className="input-light" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Email *</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@elbold.com" className="input-light" required />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          <h2 className="text-base font-semibold text-gray-900 mb-1">Admin Panel Test Results</h2>
          <p className="text-sm text-gray-400 mb-5">Mark each admin function PASS or FAIL based on what you tested.</p>
          <div className="space-y-3">
            {ADMIN_STEPS.map(({ key, label }) => (
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div>
            ...
            <textarea value={comments} onChange={(e) => setComments(e.target.value)} rows={4} placeholder="What worked well? What needs improvement? Any critical blockers?" className="input-light resize-none" />
          </div>
        </div>
            ...
            <div className="space-y-4 pt-4 border-t border-gray-100">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={bugUrl} onChange={(e) => setBugUrl(e.target.value)} placeholder="https://www.elbold.com/admin/..." className="input-light" />
                </div>
                <div>
