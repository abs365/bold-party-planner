  ...
  return (
    <div className="space-y-4">
      {quotes.map((quote) => {
        ...
        return (
          <Link key={quote.id} href={`/dashboard/quotes/${quote.id}`} className="glass-card p-4 flex gap-4 card-hover block">
            {cover ? (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-white/50 text-sm">{quote.event?.title ?? quote.event_type ?? "Event"}</p>
                </div>
              ...
              <div className="flex items-center justify-between mt-2">
                <p className="text-white/40 text-xs">
                  ...
                  {quote.budget_min != null && ` · Budget £${quote.budget_min}–£${quote.budget_max ?? "open"}`}
                </p>
                {response && (
                  <span className="text-purple-300 text-sm font-semibold">£{response.price.toFixed(2)}</span>
                )}
              </div>
            </div>
