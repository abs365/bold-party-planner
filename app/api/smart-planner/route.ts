...
export async function POST(request: Request) {
  ...
  const rlMin = await rateLimit({ identifier: `smart-planner:min:${identifier}`, limit: 10, windowMs: 60_000 });
  const rlHour = await rateLimit({ identifier: `smart-planner:hr:${identifier}`, limit: 50, windowMs: 60 * 60_000 });
