...
> **Enterprise Baseline v1.0.1 Correction (2026-07-10):** the paragraph above is disproved. Direct production verification (schema inspection + a rolled-back live write test) confirms `vendors.response_rate` is `NUMERIC(5,2)` with no CHECK constraint and accepts 0-100 writes without error. Level 3 auto-upgrade is not blocked by this mechanism. Full evidence: `ELBOLD_ENTERPRISE_BASELINE_v1.0.1_CORRECTIONS.md`. The verification-credential argument in Section 1.2 stands without qualification.
