...
> **Enterprise Baseline v1.0.1 Correction (2026-07-10):** the finding immediately above was disproved by direct production verification before implementation. Live `vendors.response_rate` is `NUMERIC(5,2)` with no CHECK constraint — a 0-100 write succeeds cleanly. No scale-mismatch bug exists. Full evidence and root-cause analysis: `ELBOLD_ENTERPRISE_BASELINE_v1.0.1_CORRECTIONS.md`. This paragraph is left in place, uncorrected, as the historical record of what v1.0 believed — do not act on it.
